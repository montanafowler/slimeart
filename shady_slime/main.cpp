...
#define FALSE_COLOR_PALETTE "data/rect815.tga"
#endif

#ifdef REGIME_BOLSHOI_PLANCK
#define DATASET_NAME "data/BP/bpdat_boxDist_trimDist_trimMass_t=0.05_subrate=1"
#define FALSE_COLOR_PALETTE "data/palette_sunset2.tga"
#endif

#ifdef REGIME_FRB
#define DATASET_NAME "data/FRB/frb_field_cigaleMass_t=10.0"
#define FALSE_COLOR_PALETTE "data/palette_hot.tga"
#endif

#ifdef REGIME_EAGLE
#define DATASET_NAME "data/eagle"
const int N_DATA_POINTS = 6381559;
const float MEAN_WEIGHT = 1.0;
#define FALSE_COLOR_PALETTE "data/palette_eagle2.tga"
#endif

#ifdef REGIME_ROCKSTAR
#define DATASET_NAME "data/rockstar"
const int N_DATA_POINTS = 3280444;
const float MEAN_WEIGHT = 1.0;
#define FALSE_COLOR_PALETTE "data/palette_magma.tga"
#endif

#ifdef REGIME_POISSON
// #define DATASET_NAME "data/Poisson/regular_4096_3d_bias=4.0"
...
int main(int argc, char **argv)
{
    ...
    config_file.open("config.txt", std::ofstream::in);
    if (!config_file.good()) {
    ...
    // sightline_x = world_to_grid(308.97582987438165, WORLD_SIZE_X, WORLD_CENTER_X, GRID_RESOLUTION_X);
    ...
    simulation_config.sense_spread = math::deg2rad(20.0);
    simulation_config.sense_distance = measure_world_to_grid(2.5, WORLD_SIZE_X, float(GRID_RESOLUTION_X));
    simulation_config.turn_angle = math::deg2rad(10.0);
    simulation_config.move_distance = measure_world_to_grid(0.1, WORLD_SIZE_X, float(GRID_RESOLUTION_X));
    simulation_config.deposit_value = 0.0;
    simulation_config.decay_factor = 0.96;
    ...
    simulation_config.move_sense_coef = 3.0;
    ...
    while(is_running)
    {
        ...
        if (show_ui) {
            ...
            ui::add_slider(&panel, "SENSE SPRD [DEG]", &ss, 0.0, 90.0);
            ...
            ui::add_slider(&panel, "TURN ANGLE [DEG]", &ts, 0.0, 45.0);
            ...
            ui::add_slider(&panel, "DEPOSIT VALUE", &simulation_config.deposit_value, 0.0, 10.0);
            ui::add_slider(&panel, "PERSISTENCE", &simulation_config.decay_factor, 0.8, 0.995);
            ui::add_slider(&panel, "CENTER ATTRACTION", &simulation_config.center_attraction, 0.0, 5.0);
            ui::add_slider(&panel, "SHARPNESS COEF", &simulation_config.move_sense_coef, 0.0001, 10.0);

            float swgt = log(rendering_config.sample_weight) / log(10.0);
            ui::add_slider(&panel, "SAMPLE WEIGHT", &swgt, -5.0, 3.0);
            rendering_config.sample_weight = math::pow(10.0, swgt);
            ui::add_slider(&panel, "DATA WEIGHT", &rendering_config.galaxy_weight, 0.0, 1.0);
            ui::add_slider(&panel, "OPTICAL THICKNESS", &rendering_config.optical_thickness, 0.0, 1.0);
            ...
            ui::add_slider(&panel, "HIGHLIGHT DENSITY", &hgd, -5.0, 9.0);
            ...
            ui::add_slider(&panel, "OVERDENSITY LOW", &odt_low, -5.0, odt_high);
            ui::add_slider(&panel, "OVERDENSITY HIGH", &odt_high, odt_low, 9.0);
            ...
            ui::add_slider(&panel, "BACKGROUND COLOR", &background_color, 0.0, 1.0);
