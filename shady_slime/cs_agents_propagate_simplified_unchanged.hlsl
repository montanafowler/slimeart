...
cbuffer ConfigBuffer : register(b0)
{
    ...
    float deposit_value; // 0 for data-driven fitting, >0 for self-reinforcing behavior
    int world_width; //
...
void main(uint thread_index : SV_GroupIndex, uint3 group_id : SV_GroupID){
    ...
    // Check if the particle is inactive and needs to be reset to a random position
    const float w_f = 0.9;
